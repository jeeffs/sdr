...
window.capturarGPS = function capturarGPS() {
  ...
  const ACC_IDEAL  = 20;    // para imediatamente se atingir ±20m
  const ACC_GOOD   = 50;    // aceita após MIN_COLLECT se accuracy < 50m
  const MIN_COLLECT= 6000;  // coleta por pelo menos 6s antes de finalizar
  const TIMEOUT_MS = 20000; // timeout absoluto — usa melhor leitura obtida
  let bestPos = null;       // melhor posição (menor accuracy) coletada
  let timerDone = false;
  ...
  _gpsWatchIdAdmin = navigator.geolocation.watchPosition(
    pos => {
      ...
      if (acc <= ACC_IDEAL || (elapsed >= MIN_COLLECT && acc <= ACC_GOOD)) {
        clearTimeout(timer); finalizarGPS(bestPos);
      }
    },
    err => {
      clearTimeout(timer);
