...
// Modal de resultado de viabilidade comercial
function _sdrViabModal(lat, lng, results, radiusM) {
  ...
  if (total === 0) {
    ...
    results.forEach(function(r) {
      ...
      var portaLabel = r.livres > 0 ? (r.livres + ' porta' + (r.livres>1?'s':'')+' livre'+(r.livres>1?'s':'')) : 'Sem porta livre';
      rows += '<div style="padding:10px 12px;border:1px solid #e5e7eb;border-radius:10px;margin-bottom:8px;cursor:pointer;transition:all .15s"'
        ...
        + ' onclick="sdrMap.flyTo([' + r.item.lat + ',' + r.item.lng + '],18);document.getElementById(\'viab-modal-ov\').remove();sdrCTOPortsModal(\'' + r.id + '\',sdrInfraCache[\'' + r.id + '\'])">'
  ...
  div.innerHTML = '<div style="padding:12px 14px;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;gap:8px">'
    ...
    + '<button class="btn-map" onclick="sdrViabilidadeCheck(' + lat + ',' + lng + ',parseInt(document.getElementById(\'viab-r-input\').value)||500)" style="flex:1;padding:6px;font-size:.78rem"><i class="fas fa-sync-alt"></i> Rebuscar</button>'
    + '</div>';
