...
(function _sdrBootstrap() {
  ...
  if (typeof PAGES !== 'undefined' && PAGES.master) {
    if (!PAGES.master.find(function(p){ return p.id === 'mapa'; })) {
      PAGES.master.push({id:'mapa', icon:'fa-network-wired', label:'Mapa de Rede'});
    }
  }
  ...
  if (typeof showPage === 'function' && !showPage._sdrPatched) {
    ...
    window.showPage = function(name) {
      _origShowPage(name);
      if (name === 'mapa' && typeof sdrMapInit === 'function') {
        sdrMapInit();
      }
    };
...
window.sdrCableDetailModal = function(id,item) {
  const tipos={backbone:'Backbone / Alimentador',distribuicao:'Distribuição',drop:'Drop / Acesso',projeto:'Projeto'};
  const sty=CABLE_RENDER[item.cable_type||'unknown']||CABLE_RENDER.unknown;
  const modal=document.createElement('div');
  ...
  modal.innerHTML=`<div style="background:#fff;border-radius:14px;width:100%;max-width:460px;box-shadow:0 10px 40px rgba(0,0,0,.25)">
    <div style="padding:14px 18px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center">
      <div style="font-weight:700;color:#1e293b">
        <span style="display:inline-block;width:24px;height:6px;background:${item.kmz_color||sty.color};border-radius:3px;margin-right:8px;vertical-align:middle"></span>
        ${item.name||'Cabo'}
      </div>
    ...
    <div style="padding:16px 18px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
        <div style="background:#f8fafc;border-radius:8px;padding:11px;text-align:center">
          <div style="font-size:.72rem;color:#64748b;margin-bottom:3px">Tipo</div>
          <div style="font-weight:700;font-size:.88rem">${tipos[item.cable_type]||'Desconhecido'}</div>
        </div>
        <div style="background:#f8fafc;border-radius:8px;padding:11px;text